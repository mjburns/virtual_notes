...
We need to be able to access two shared drives. The first is the Medixflux store. The other store is the WERG shared drive. The code below needs to be run in the Linux command line. In this example code below, the Mediaflux store will be mounted to the folder `mnt` in the `HOME` directory. The WERG drive will be mounted in the directory `mnt2`. Of course, choose your preferred directory names.

`sudo mount -t cifs -ouser=burnsm,domain=unimelb,uid=$UID,gid=$(id -g) //mediaflux.researchsoftware.unimelb.edu.au/proj-6600_monbulk_platypus-1128.4.857 $HOME/mnt`
